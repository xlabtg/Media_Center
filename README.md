...
| [docs/MINIMAL_PRODUCTION_RESOURCES.md](docs/MINIMAL_PRODUCTION_RESOURCES.md) | Минимальный production capacity-профиль `nmc-minimal-100upm`: сервисы, CPU/RAM и инфраструктура |
...
| [docs/CHAOS_TESTING.md](docs/CHAOS_TESTING.md) | Chaos-контракт отказов PostgreSQL, RabbitMQ, external API и proxy |
...
Chaos-контур issue #89 добавляет [docs/CHAOS_TESTING.md](docs/CHAOS_TESTING.md)
и `libs.shared.resilience` для воспроизводимых отказов PostgreSQL, RabbitMQ,
external API и proxy с retries, timeout budget, controlled degradation и
recovery confirmation.
Backup/DR-контур issue #99 добавляет
[docs/DISASTER_RECOVERY.md](docs/DISASTER_RECOVERY.md),
`infra/backup/backup-policy.json`, cron-шаблон и dry-run команды
`make backup-policy`, `make backup-local`, `make restore-drill` для
PostgreSQL, ChromaDB и S3/MinIO.
