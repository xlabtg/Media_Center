...
Для issue #64 добавлен backend-контур Agentic RAG / DeepResearch / Content
Agent (CUA): tenant-scoped документы загружаются в общий vector store, RAG
возвращает контекст только в пределах tenant, исследовательский режим формирует
черновики, а CUA-действия остаются proposal до human approval.

Для issue #65 добавлен XAI-аудит решений AI: каждый `AgentRun` сопровождается
`DecisionExplanation`, объяснения доступны Совету через tenant-scoped журнал и
фиксируются в audit metadata через hash без raw-ссылок.
...
- Agentic RAG возвращает tenant-scoped контекст через `agentic_rag`, используя
  документы, добавленные в `/rag/documents`, и не смешивает результаты разных
  tenant.
- DeepResearch формирует черновик материала через `deep_research`, прикладывает
  citations из RAG-контекста и помечает результат как требующий человеческой
  проверки.
- Content Agent (CUA) не исполняет действия без human approval:
  `content_agent_action` всегда возвращает `awaiting_human_approval`,
  `requires_human_approval=true` и `auto_executed=false`.
- XAI-аудит решений AI добавляет к каждому `AgentRun` поле
  `decision_explanation` с summary, reason codes, безопасными input facts,
  evidence/action refs, `explanation_hash` и `audit_hash`.
- `GET /agents/explanations` возвращает tenant-scoped журнал объяснений для
  Совета, Президиума и Правления; участники без управленческой роли получают
  `403 forbidden`.
- Audit/events для AI-решений содержат только `decision_explanation_id` и
  `decision_explanation_hash`, без raw recipient refs, content, source refs,
  workspace refs или target refs.
...
- `POST /rag/documents` добавляет tenant-scoped документы в RAG vector store.
- `POST /agents/run` запускает `audience_analysis`, `engagement_auto_reply`,
  `content_hygiene`, `publication_optimization`, `agentic_rag`,
  `deep_research` или `content_agent_action`.
- `GET /agents/status?task_type=` возвращает историю запусков tenant.
- `GET /agents/explanations?task_type=` возвращает XAI-журнал объяснений
  решений AI для Совета.
