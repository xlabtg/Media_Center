...
для #69, онбординга для #70, UI голосового ассистента для #72,
дизайн-системы для #73 и privacy self-service контура ФЗ-152 для #87. Он собирает
...
Для аудита ФЗ-152 сервис также публикует checklist, data map, реестр согласий
и DSAR workflow для доступа, исправления, удаления и прекращения необязательной
обработки.
...
- `GET /compliance/fz152/checklist` возвращает исполняемый checklist ФЗ-152 с
  evidence refs для текущего tenant.
- `GET /privacy/data-map` возвращает категории ПДн, цели, основания, сроки
  хранения, получателей и стратегию удаления.
- `GET /privacy/consents` возвращает реестр согласий участника: version,
  purpose, legal basis, retention policy, allowed actions и статус отзыва.
- `POST /privacy/data-subject-requests` регистрирует DSAR-запрос и исполняет
  удаление projection-данных или отзыв согласий, если это применимо.
- `GET /privacy/data-subject-requests/{request_id}` возвращает журналированное
  обращение субъекта ПДн с `audit_hash`.
...
- `OnboardingConsentRecord` для #87 хранит `consent_version`, `purpose`,
  `legal_basis`, `retention_policy`, `allowed_actions` и `revoked_at`, чтобы
  согласия были проверяемыми и отзыв менял необязательные сценарии.
- `DataSubjectRequestRecord` журналирует обращения субъекта ПДн, удалённые
  ресурсы, отозванные согласия, retained resources и hash-only evidence.
...
- ФЗ-152 endpoints для #87 tenant-aware: участник видит только свои согласия и
  обращения, а `council`/`presidium`/`board` могут работать с участником внутри
  своего tenant.
- `POST /privacy/data-subject-requests` с `request_type=erasure` удаляет Web
  Cabinet projection-данные участника и оставляет только hash-only/legal
  retention evidence; `processing_restriction` отзывает выбранные согласия.
