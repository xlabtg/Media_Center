...
Некоторые сервисы переиспользуют доменные модели соседних сервисов в памяти,
например `web-cabinet`, `voice-to-chain` и `cglr`. Поэтому Dockerfile копирует
в `/app/service` все легковесные peer service packages из `services/`, но не
добавляет их внешние тяжелые зависимости в runtime venv. Это сохраняет
работоспособность `/ready` и API-import contract без возврата к установке всего
`[project].dependencies`.

Чтобы cold-start не тратил секунды на компиляцию Python-модулей, build stage
выборочно создаёт bytecode для FastAPI/Starlette/Pydantic, текущего service
package, его `*_app` entrypoint и `libs/shared`. Полный venv bytecode не
сохраняется: он выводит образ за 250 МБ, тогда как selective bytecode оставляет
гейт в пределах бюджета.
