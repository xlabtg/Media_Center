...
| `CONFIG_SERVER_URL`, `CONFIG_SERVER_ENV`, `CONFIG_SERVER_TOKEN_PATH`, `CONFIG_SERVER_TIMEOUT_SECONDS` | Опциональный config server в Kubernetes |
...
- Если `CONFIG_SERVER_URL` задан и доступен Kubernetes ServiceAccount token,
  runtime-конфигурация загружается из config server с `Authorization: Bearer`
  и имеет приоритет над env-переменными. `project`, `framework` и `application`
  задаются в коде сервиса; `CONFIG_SERVER_ENV` выбирает runtime-окружение.
