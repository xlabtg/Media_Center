...
| `CONFIG_SERVER_URL`, `CONFIG_SERVER_PROJECT`, `CONFIG_SERVER_ENV`, `CONFIG_SERVER_FRAMEWORK`, `CONFIG_SERVER_TOKEN_PATH`, `CONFIG_SERVER_TIMEOUT_SECONDS` | Опциональный config server в Kubernetes |
...
- Если `CONFIG_SERVER_URL` задан и доступен Kubernetes ServiceAccount token,
  ...
  и имеет приоритет над env-переменными.
