...
| Задача | Статус | Проверяемые артефакты |
| --- | --- | --- |
| F4 / #254 | Выполнено: `api-gateway`, `contribution-ledger` и `wallet` имеют формализованные availability SLO, latency p95, 30-дневный error budget, fast/slow burn rate alerts и runbook для triage. | `infra/observability/slo-targets.json`, `infra/observability/prometheus/rules/slo-error-budget.yml`, `docs/operations/slo-error-budget.md`, `tests/test_slo_error_budget_issue254_contract.py` |
...
Контракт F4 по issue #254 закреплён в
`tests/test_slo_error_budget_issue254_contract.py`. Он проверяет, что:

- `infra/observability/slo-targets.json` содержит REQ-N5, issue #254,
  `api-gateway`, `contribution-ledger`, `wallet`, их SLI/SLO, 30-дневный error
  budget и fast/slow burn policy;
- `infra/observability/prometheus/rules/slo-error-budget.yml` считает error
  ratio, нормализованный burn rate по окнам `5m`, `30m`, `1h`, `6h`, и заводит
  availability, latency p95, fast burn и slow burn alerts;
- `docs/operations/slo-error-budget.md`, `docs/SRE_RUNBOOK.md` и
  `infra/observability/README.md` связывают SLO, Alertmanager routing,
  runbooks и локальную проверку.
