...
| SLO доступности | F4 / #254 задает 99,9 % для `api-gateway`, 99,5 % для `contribution-ledger` и `wallet`; общий SRE-каталог также сохраняет цели для `hitl-payout-gateway`, `messenger-adapter`, `blockchain-auditor` и `observability`; error budget от 0,1 % до 1,0 %. | Формализованный SLO + error budget; ключевой публичный edge target 99,9 % и service-specific burn rate alerts. | `infra/observability/slo-targets.json`, `infra/observability/prometheus/rules/slo-error-budget.yml`, `docs/operations/slo-error-budget.md`, `docs/SRE_RUNBOOK.md`, Prometheus SLO alerts и incident process. | REQ-N5, F4 / #254, REQ-M4 |
