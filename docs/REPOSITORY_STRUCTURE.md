...
Каркас каталогов готов. Исполняемый код, `pyproject.toml`, lock-файлы,
docker-compose и CI добавляются отдельными задачами этапов 0-1, чтобы не
смешивать структуру репозитория с реализацией сервисного шаблона.
