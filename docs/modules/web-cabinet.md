...
**Статус:** 🟢 реализовано для #67, #68, #69, #70, #72, #73, #74 и #87 · **Этап:** Этап 4 — Клиентские приложения и UX · **Компонент:** `component:web-cabinet`
...
Новая дизайн-система #73 добавляет исполняемый `nmc-ui` контракт поверх UX baseline:
токены дизайна, переиспользуемые UI-компоненты, общий CSS layer и HTML UI-kit
для всех клиентских экранов Web Cabinet.
Родительский эпик #74 закреплён через сквозной stage-4 acceptance contract:
кабинет пайщика, панель Совета, KPI-дашборд, онбординг, Telegram-клиент,
voice UI и дизайн-система проверяются как единый клиентский UX-пакет.
Для #87 Web Cabinet дополнен privacy self-service контуром ФЗ-152: исполняемый
checklist, data map, реестр версионированных согласий и DSAR workflow для
доступа, исправления, удаления и ограничения обработки.
...
добавлен в #72.
Исторический маркер голосового UI: реализовано для #67, #68, #69, #70 и #72.
...
- ФЗ-152 privacy self-service: checklist соответствия, data map, реестр
  согласий и журналируемые запросы субъекта ПДн.
...
- **GET** `/design-system/tokens` — JSON-контракт дизайн-токенов,
  компонентов, гайдлайнов и accessibility baseline `nmc-ui`.
- **GET** `/design-system/ui-kit` — HTML-витрина UI-kit с теми же токенами,
  компонентами и `data-component` markers.
- **GET** `/compliance/fz152/checklist` — исполняемый checklist ФЗ-152 с
  evidence refs для #87.
- **GET** `/privacy/data-map` — категории ПДн, цели, основания, сроки хранения,
  получатели и стратегия удаления.
- **GET** `/privacy/consents` — реестр согласий участника с версией, целью,
  правовым основанием, сроком хранения, статусом отзыва и allowed actions.
- **POST** `/privacy/data-subject-requests` — DSAR workflow для доступа,
  исправления, удаления projection-данных и ограничения обработки.
- **GET** `/privacy/data-subject-requests/{request_id}` — чтение
  журналированного обращения субъекта ПДн.
...
- **PrivacyConsentRegistryResponse** — версия согласия, цель, основание,
  retention policy, allowed actions, `granted_at` и `revoked_at`.
- **DataSubjectRequestRecord** — журнал DSAR-обращения с типом запроса,
  статусом, удаленными ресурсами, отозванными согласиями, retained resources и
  `audit_hash`.
...
- **VoiceAssistantTranscriptionRequest / Voice-to-Chain receipt** — UI-запрос
  ...
  Cabinet.
- **DesignSystemResponse** — контракт #73 для токенов, компонентного каталога,
  гайдлайнов и accessibility baseline, реализованный в
...
- ФЗ-152 endpoints доступны tenant-aware: участник работает только со своими
  согласиями и DSAR-запросами, роли `council`, `presidium`, `board` могут
  открыть участника внутри tenant.
- `POST /privacy/data-subject-requests` для `erasure` удаляет Web Cabinet
  projection-данные участника (`cabinet_content`, `cabinet_contributions`,
  onboarding profile/steps/consents) и сохраняет только hash-only/legal
  retention evidence.
- `processing_restriction` отзывает выбранные согласия и меняет доступные
  необязательные сценарии без затрагивания данных другого tenant.
...
- [tests/test_design_system_issue73_acceptance_contract.py](../../tests/test_design_system_issue73_acceptance_contract.py) —
  acceptance-тест #73.
- [tests/test_stage4_acceptance_contract.py](../../tests/test_stage4_acceptance_contract.py) —
  сквозной stage-4 acceptance contract для epic #74.
- [tests/test_compliance_issue87_contract.py](../../tests/test_compliance_issue87_contract.py) —
  acceptance-тест аудита ФЗ-152 #87.
- [docs/modules/design-system.md](design-system.md) — спецификация токенов,
  компонентов и доступности дизайн-системы #73.
- [docs/STAGE_4_ACCEPTANCE.md](../STAGE_4_ACCEPTANCE.md) — acceptance snapshot
  этапа 4.
...
- [#71](https://github.com/xlabtg/Media_Center/issues/71) — Telegram-клиент (шифрование, прокси)
...
- [#74](https://github.com/xlabtg/Media_Center/issues/74) — Этап 4 — Клиентские приложения и UX
- [#87](https://github.com/xlabtg/Media_Center/issues/87) — Аудит соответствия ФЗ-152 (ПДн)
- [#60](https://github.com/xlabtg/Media_Center/issues/60) — Wallet Module: учёт МСЦ и операций

---
<sub>Спецификация синхронизирована с реализацией Web Cabinet для issue #67, #68, #69, #70, #72, #73, #87 и сквозным stage-4 acceptance contract #74.</sub>
