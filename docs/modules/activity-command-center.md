...
## Связь с панелью Совета #68

#68 реализует клиентский экран поверх уже готовых backend-контуров: Web Cabinet
показывает очередь HITL, окно вето, политику `hitl.veto_window_hours`,
audit timeline и 2FA-dialog подтверждения выплат. Activity Command Center
остаётся backend-источником порогов, очередей задач и контуров обратной связи.
