...
## Реализовано для issue #223

- `AppSettings.app_port` по умолчанию использует единый runtime-порт `7700`;
- `APP_PORT` и `LOG_LEVEL` переопределяются через env и проходят типовую
  валидацию Pydantic Settings;
- `LOG_LEVELS` содержит полный набор `DEBUG`, `INFO`, `WARNING`, `ERROR`,
  `CRITICAL`, а `LOG_LEVEL` нормализуется к верхнему регистру.
